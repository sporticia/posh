...
{
    $ImageName = (Get-AzureVMImage | Where-Object { $_.Label -like "*Server 2016*"} | Sort-Object PublishedDate -Descending | Select-Object -ExpandProperty ImageName -First 1)
    Return $ImageName
}

# Get latest Windows OS (ARM)
Function GetLatestARMOS
{
...
Get-AzureVMImage | Where-Object {($_.Label -like "*SQL Server 2016*") -and ($_.Label -like "*Server 2016*")} | Select-Object Label
...
Function Send-MailgunMsg
{
  ...
  $From = "Alert <powershell@org.com>"
  $To = $To -join "&To="
  $APIKEy = "<key>"
  $API = "https://api.mailgun.net/v2/<org>/messages"
...
{
  $url = "https://login.microsoftonline.com/<GUID>/oauth2/token"
...
Function Login-and-Select-Subs($SpnUser, $SpnPassowrd, $Subscription )
{
  ...
  If (!(([string]::IsNullOrEmpty($SpnUser)) -or ([string]::IsNullOrEmpty($SpnPassowrd))))
  {
      ...
      Login-AzureRmAccount -ServicePrincipal -Credential $spn -TenantId "<GUID>"
...
Connect-AzureAD
Get-AzureADObjectByObjectId -ObjectIds "<ObjId>"
...
#Get-PrivateKey -PFXFile "c:\temp\certificate.pfx" -Password (ConvertTo-SecureString -Force -AsPlainText "password")
#Get-PrivateKey
...
#((Find-Package -Source https://org.com/guestAuth/app/nuget/v1/Service.svc).Name)
